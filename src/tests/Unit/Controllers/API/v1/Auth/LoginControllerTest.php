...
class LoginControllerTest extends TestCase
{
    ...
    private function createAndMockUserAndRequestForRefreshToken(string $userEmail, bool $shouldTokenDeletionFail, bool $shouldTokenCreationFail): Request
    {
        ...
        // Mock the currentAccessToken and revoke calls
        $userMock->shouldReceive('currentAccessToken')->once()->andReturnSelf();
        if ($shouldTokenDeletionFail) {
            $userMock->shouldReceive('revoke')->once()->andThrow(new Exception('Token deletion failed'));
        } else {
            $userMock->shouldReceive('revoke')->once()->andReturn(true);
        }
        ...
        if (!$shouldTokenDeletionFail) {
            ...
            if ($shouldTokenCreationFail) {
                ...
                $accessToken = bcrypt($userEmail);
                $userMock->shouldReceive('createToken')->once()->with($userEmail)->andReturn((object) ['accessToken' => $accessToken]);
            }
        }
    ...
    public function testLoginSucceedsWhenValid(): void
    {
        ...
        $payload = ['email' => 'correct@example.com', 'password' => 'secret'];
        $request = $this->prepareRequest($payload);
        $accessToken = bcrypt($payload['email']);
        ...
        $expected = (object) [
            ...
            'token_type' => 'Bearer',
            'expires_in' => config('passport.personal-access-token-expiration') * 24 * 60 * 60,
        ...
        $user->password = bcrypt($payload['password']);
        $user->shouldReceive('hasVerifiedEmail')->andReturn(true);
        $user->shouldReceive('createToken')->andReturn((object) ['accessToken' => $accessToken]);
